...
const LOG_SUBDIR: &str = "nixblitz_cli";
...
fn get_default_log_file() -> PathBuf {
    ...
    match state_dir {
        ...
        None => {
            panic!(
                "Could not determine the user's state directory (e.g., ~/.local/state). Ensure $XDG_STATE_HOME environment variable is set or pass `--log-file`"
            );
        }
    }
}
...
mod tests {
    ...
    fn test_get_log_level_env() {
        ...
        unsafe { std::env::set_var(LOG_LEVEL_ENV, "ERROR") };
        assert_eq!(get_log_level(), LevelFilter::Error);

        unsafe { std::env::set_var(LOG_LEVEL_ENV, "WaRn") };
        assert_eq!(get_log_level(), LevelFilter::Warn);

        unsafe { std::env::set_var(LOG_LEVEL_ENV, "invalid") };
        assert_eq!(get_log_level(), DEFAULT_LOG_LEVEL);

        if let Some(original) = original_log_level {
            unsafe { std::env::set_var(LOG_LEVEL_ENV, original) };
        } else {
    ...
    fn test_log_file_override_priority() {
        ...
        let cli_path_str = "/tmp/nixblitz_cli.log";
        let cli_path = PathBuf::from(cli_path_str);

        // default only
        unsafe { std::env::remove_var(LOG_FILE_ENV) };
        let cli_args_1 = Cli {
            command: None,
            log_file: None,
            log_level: None,
        ...
        unsafe { std::env::set_var(LOG_FILE_ENV, env_path.to_str().unwrap()) };
        let cli_args_2 = Cli {
            command: None,
            log_file: None,
            log_level: None,
        ...
        if let Some(original) = original_env_var {
            unsafe { std::env::set_var(LOG_FILE_ENV, original) };
        } else {
            unsafe { std::env::remove_var(LOG_FILE_ENV) };
        }
    }
