...
def main [
  commit_sha?: string # The optional commit SHA to use. If not provided, it uses the latest git commit.
    ...
    let files = [
        ["./crates/nixblitz_cli/default.nix", ".#nixblitz-cli"]
        ["./crates/nixblitz_norupo/default.nix", ".#nixblitz-norupo"]
    ]

    for file in $files {
        let file_path = $file | get 0
        let target = $file | get 1
        print $"INFO: Updating default.nix for target: ($target)"

        if not ($file_path | path exists ) {
            print "ERROR: Filepath not found: ($file)"
            exit 1
        }
        ...
        let build_result =  nix build $target | complete
        ...
        let final_build = nix build $target | complete
