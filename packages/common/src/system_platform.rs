...
impl SystemPlatform {
    ...
    pub fn from_short_str_option(s: &str) -> Result<SystemPlatform, String> {
        ...
        Err(format!("Unable to parse platform from string: {}", s))
    }
}
...
mod tests {
    ...
    fn test_from_short_str_option() {
        ...
        assert_eq!(
            ...
            Ok(SystemPlatform::Pi5)
        );
        assert_eq!(
            SystemPlatform::from_short_str_option("unknown_platform"),
            Err(format!(
                "Unable to parse platform from string: {}",
                "unknown_platform"
            ))
        );
    }
